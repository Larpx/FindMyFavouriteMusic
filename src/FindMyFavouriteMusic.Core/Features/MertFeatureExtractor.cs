...
public class MertFeatureExtractor : IDeepFeatureExtractor
{
    ...
    public async Task<Result<float[]>> ExtractAsync(float[] samples, int sampleRate, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Result.Failure 通常是业务逻辑问题（如所有帧推理失败但非 EP 异常），不触发 EP 回退
            _logger.LogError("MERT 深度特征提取失败: {Error}", result.Error);
            return result;
        }
        catch (Exception ex)
        {
            // 异常通常是 EP 算子不兼容（如 Reshape 节点 RuntimeException），尝试 CPU 回退
            if (!CanFallbackToCpu())
            {
            ...
            _logger.LogWarning(ex, "MERT 推理异常（EP={EP}），回退到 CPU EP 并重试",
            ...
            try
            {
                ...
                {
                    _logger.LogInformation("CPU EP 回退后 MERT 推理成功");
                }
                return retryResult;
            }
            catch (Exception retryEx)
            {
                _logger.LogError(retryEx, "CPU EP 回退后 MERT 推理仍失败");
    ...
    private bool TryRebuildSessionWithCpu()
    {
        ...
        try
        {
            ...
            _logger.LogInformation("已用 CPU EP 重建 MERT 推理会话");
            return true;
        }
        catch (Exception ex)
        {
