...
本文档说明 **Find My Favourite Music** 中从音频文件到匹配分数的**数据处理与算法原理**（解码、特征、画像、相似度、向量序列化、EP 选型等），内容基于 `src/` 实际源码。

相关文档：

- [用户使用说明](用户使用说明.md) — 一般用户操作
- [开发说明](开发说明.md) — 构建、架构、测试与扩展
- [README](../README.md) — 仓库入口

> 命名空间约定：`Larpx.PersonalTools.FindMyFavouriteMusic.{Layer}`，由 `src/Directory.Build.props` 中的 `RootNamespace` 拼接。
...
## 10. Execution Provider 性能对比与选型

> 测试时间：2026-07-25
> 测试脚本：`Run-Ep-Benchmark.ps1`
> 测试音频：`Models/ナナツカゼ,PIKASONIC,なこたんまる - 再生.flac`（约 161 秒，重采样至 16kHz 单声道后 2,582,069 个采样点）

### 10.1 测试背景

ONNX Runtime 支持多种 Execution Provider（EP）来加速模型推理。本项目最初采用 DirectML EP（Windows 通用加速方案），但发现 MERT 模型含动态形状 Reshape 算子，DirectML EP 推理必失败触发 CPU 回退。因此引入 OpenVINO EP（Intel 官方最优方案）进行对比测试，决定最终保留哪种 EP。
...
|------|---------|----------|------|
| CPU | CPU | - | 纯 CPU 推理基线 |
| DirectML | DirectML | - | Windows 通用加速，自动 offload 到 NPU/GPU |
| OpenVINO-AUTO | OpenVINO | AUTO | OpenVINO 运行时自动选择最佳设备 |
| OpenVINO-NPU | OpenVINO | NPU | Intel AI Boost NPU |
| OpenVINO-GPU | OpenVINO | GPU | Intel Arc 集成 GPU |

### 10.2 测试结果

| 配置 | VGGish 加载 (ms) | VGGish 推理 (ms) | VGGish 实际 EP | MERT 加载 (ms) | MERT 推理 (ms) | MERT 实际 EP |
|------|-----------------|-----------------|---------------|----------------|----------------|--------------|
| CPU | 604 | **2561** | CPU | 574 | 36585 | CPU |
| DirectML | 768 | 4312 | DirectML | 1116 | 38531 | CPU（回退） |
| OpenVINO-AUTO | 680 | 3063 | OpenVINO(AUTO) | 686 | 33310 | OpenVINO(AUTO) |
| OpenVINO-NPU | 564 | 5133 | OpenVINO(NPU) | 432 | 38757 | OpenVINO(NPU) |
| OpenVINO-GPU | 696 | 5430 | OpenVINO(GPU) | 413 | **16308** | OpenVINO(GPU) |

### 10.3 关键发现

**1. VGGish 不需要加速**

VGGish 是 128 维输出的小模型，CPU 已最快（2561ms）。所有加速 EP 因数据搬运/调度开销反而比 CPU 慢 1.2–2.1 倍：

| EP | VGGish 推理 | 相对 CPU |
|----|------------|---------|
| CPU | 2561ms | 1.00x（基线） |
| OpenVINO-AUTO | 3063ms | 0.84x（更慢） |
| DirectML | 4312ms | 0.59x（更慢） |
| OpenVINO-NPU | 5133ms | 0.50x（更慢） |
| OpenVINO-GPU | 5430ms | 0.47x（更慢） |

**2. MERT 必须用 OpenVINO(GPU)**

MERT 是 768 维输出的大模型，GPU 能有效加速：

| EP | MERT 推理 | 相对 CPU | 关键问题 |
|----|----------|---------|---------|
| OpenVINO-GPU | 16308ms | **2.24x** | 唯一有效加速 |
| OpenVINO-AUTO | 33310ms | 1.10x | AUTO 默认偏 NPU，未选到 GPU |
| CPU | 36585ms | 1.00x（基线） | - |
| DirectML | 38531ms | 0.95x（更慢） | 触发 CPU 回退，含重建会话开销 |
| OpenVINO-NPU | 38757ms | 0.94x（更慢） | NPU 编译开销大，不适合大模型 |

**3. DirectML 对 MERT 完全无效**

DirectML EP 不支持 MERT 的动态形状 Reshape 算子，推理必失败触发 CPU 回退。回退路径包含"DirectML 失败 + 重建 CPU 会话 + CPU 推理"，耗时 38531ms，比直接 CPU 推理（36585ms）还慢 1946ms。

### 10.4 最优方案选型

综合推荐质量与推理速度，**最优方案为 MERT + OpenVINO(GPU) + 启用编译缓存目录**：
...
| 维度 | MERT + OpenVINO(GPU) | VGGish + CPU |
|------|---------------------|--------------|
| 推荐质量 | **优**（768 维音乐专用特征） | 一般（128 维通用音频特征） |
| 单首推理耗时 | 16.3s | **2.6s** |
| 模型适配性 | MERT 是音乐自监督模型，专为音乐任务设计 | VGGish 是 Google Audioset 通用模型 |
| EP 稳定性 | OpenVINO(GPU) 直接推理成功，无回退 | CPU 原生支持，零风险 |

1. **质量优先**：本程序核心价值是"找我最爱的音乐"，推荐质量是首要目标。MERT 768 维特征比 VGGish 128 维更能捕捉音乐风格相似性。
2. **速度可接受**：MERT + OpenVINO(GPU) 16.3s/首。首次全库扫描后入库缓存，后续无需重复推理。
3. **OpenVINO 缓存二次加速**：配置 `OpenVinoCacheDir` 后，模型编译结果缓存到磁盘，二次启动跳过编译阶段。
4. **DirectML 应淘汰**：对 VGGish 比 CPU 慢 1.7x，对 MERT 触发 CPU 回退，在两种模型上均无优势。

### 10.5 后续架构决策

基于测试结果，v2.0 版本起移除 DirectML EP 依赖，仅保留 OpenVINO + CPU 双 EP 架构：

- **CPU EP**：适用于 VGGish 等小模型，或无 Intel GPU/NPU 的环境
- **OpenVINO EP**：适用于 MERT 等大模型，支持 NPU/GPU/AUTO 三种目标设备（默认 GPU）
- 用户可通过设置页 UI 在 CPU 与 OpenVINO 之间切换，并选择 OpenVINO 目标设备

---

**文档版本**：基于项目源码撰写，最后核对日期 2026-07-25。
