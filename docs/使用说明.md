...
| 能力 | 说明 |
|------|------|
| 音乐库管理 | 扫描本地目录，MD5 缓存跳过未变文件；右侧详情编辑标签/封面并写回原文件 |
| 声学特征提取 | MFCC + 频谱质心 + 色度，共 52 维 |
| 深度特征提取（可选） | VGGish（128 维）或 MERT（768 维）ONNX 模型 |
| 用户画像构建 | 从"喜欢"歌曲聚合均值向量；Welford 增量；空喜欢清空画像 |
...
覆盖约定与 Hardware 测试说明见 [测试与覆盖说明](测试与覆盖说明.md)。
...
- 若 `appsettings.json` 中 `OnnxModel.EnableDeepFeatures` 为 `true` 且指定了模型路径，会尝试自动加载深度模型（VGGish / MERT）；加载失败仅记录警告，不影响应用启动。
...
- 点击图标切换喜欢状态
- **标记喜欢**：触发画像增量更新（Welford 算法）；若已是喜欢状态则为 no-op
- **取消喜欢**：触发画像全量重建；若已无喜欢歌曲则**清空画像**

**歌曲详情与标签编辑**：

1. 单击列表中的歌曲（或右键「查看详情 / 编辑标签」）→ 右侧打开详情面板
2. 封面与标签字段从**原文件实时读取**（TagLibSharp）；封面**不写入数据库**
3. 可编辑：标题、艺术家、专辑、专辑艺术家、流派、年份、曲目号、碟号、注释、歌词、封面
4. 点击「保存到原文件」：直接写回音频文件标签 → 同步 DB 文本镜像 → **更新 FileMd5，保留特征向量**
5. 只读文件会禁用保存；有未保存修改时切换歌曲会确认

**加载已有歌曲**：若数据库中已有歌曲记录，可点击"刷新列表"直接展示；再次扫描同一目录时，MD5 未变且契约匹配会跳过解码。

> **硬限制**：单文件超过 **200MB** 会在解码前被拒绝。
...
| 配置 | 操作 | 持久化位置 |
|------|------|-----------|
| 声学 / 深度 / 仅声学权重 | 调整数值 → 点击"保存权重" | AppData `usersettings.json` |
| ONNX 模型路径 / 类型 / EP / 设备 / 缓存目录 | 填写 → 点击"保存配置" | 同上 |
| 扫描最大并发数 | 调整 → 点击"保存扫描配置" | 同上 |
| 加载模型 | 点击"加载模型"（立即生效） | 内存 |
| 重建画像 | 点击"重建画像" | `UserProfile` 表 |

> **权重建议**：声学权重 + 深度权重之和应为 1.0。默认 0.4 + 0.6 = 1.0。
>
> **保存 vs 加载**：保存写入配置文件；加载是即时把模型载入内存。切换 EP 需重启应用。

---

## 6. 配置说明

配置文件位于 `src/FindMyFavouriteMusic.GUI/appsettings.json`，运行时复制到输出目录。  
**用户运行时设置**写入 `%AppData%/FindMyFavouriteMusic/usersettings.json`（原子替换 + 写锁）；首次启动若仅有应用目录旧版文件会自动迁移。
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `SupportedExtensions` | string[] | `[".mp3", ".wav", ".flac", ".ogg", ".m4a"]` | 扫描时识别的音频扩展名 |
| `MaxConcurrentProcessing` | int | 2 | 最大并发解码+特征提取数 |
| `LastScanDirectory` | string? | null | 上次扫描目录（启动时可自动再扫） |
...
2. 应用目录旧版 `usersettings.json`（兼容迁移前配置）
3. `%AppData%/FindMyFavouriteMusic/usersettings.json`（用户运行时设置，UI 修改写于此）
4. 环境变量（前缀 `FINDMYFAVOURITEMUSIC_`）
...
**A**：
- 增大 `Scan.MaxConcurrentProcessing`（默认 2，设置页可调 1~32），但注意 CPU 核心数与内存限制
- 启用深度特征会显著增加单文件耗时（ONNX 推理为 CPU/NPU 密集型），初次扫描可先关闭，扫描完成后再补提
- 内容未变（MD5 相同）且特征契约匹配的文件会跳过重算；仅深度模型切换时才补全深度向量

### Q6：修改了配置不生效？

**A**：检查清单：
1. **默认配置**：修改的是输出目录下的 `appsettings.json`，而非源码目录中的？
2. **用户运行时设置**：UI 写入的是 `%AppData%/FindMyFavouriteMusic/usersettings.json`（不是输出目录）；确认未被环境变量覆盖
3. `reloadOnChange: true` 已开启，但部分配置（如 `FeatureExtraction`）只在服务构造时读取一次，需重启应用
4. 环境变量优先级最高，确认未误设 `FINDMYFAVOURITEMUSIC_*` 变量覆盖配置
...
测试项目位于 `src/FindMyFavouriteMusic.Tests/`，使用 xUnit + FluentAssertions + Moq。覆盖约定与 Hardware 测试说明见 [测试与覆盖说明](测试与覆盖说明.md)。

### 11.2 测试覆盖范围（摘要）

| 类别 | 测试内容 |
|------|----------|
| Core 工具 | 相似度、向量序列化、聚合、格式检测、预处理、归一化、MD5、模型锁、解码硬限 |
| Services | 扫描/MD5 缓存、喜欢与画像、预测（含 BLOB 画像）、设置 AppData、标签读写、仓储迁移 |
| Hardware | 本机 ONNX/OpenVINO，日常 CI 可过滤 |

测试命名遵循 `Method_Scenario_ExpectedResult` 约定，模式为 AAA（Arrange-Act-Assert）。

---

**文档版本**：基于项目源码撰写，最后核对日期 2026-07-28。
